/* ============================================================================
   檔名：schema.sql
   用途：投智 AI 模擬投資系統 — SQL Server (T-SQL) 完整建表腳本
   目標資料庫系統：Microsoft SQL Server（用 SSMS 連線）
   來源：將 MySQL 版 schema（專題資料庫_ai建議.txt）改寫為正確 T-SQL 語法
   ...
   如何執行：
     ...
     3. 直接整份貼上 → 按「執行 (F5)」。
     4. 看訊息視窗最後的驗證結果（Section 5）確認 13 張表都建立成功。

   如何驗證：
     腳本最後（Section 5）會自動列出所有資料表名稱與資料筆數，
     正常應看到 13 張資料表、筆數皆為 0。
...
/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   下面整段被 /* */ 包起來而停用。
   只有在「要把 13 張表全部刪掉重建」且「已備份」時，才手動把這段的
...
/* ─────────────────────────────────────────────────────────────────────────
   6. portfolios — 模擬投資組合（與 users 1:1）
   ───────────────────────────────────────────────────────────────────────── */
